...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
        ...
        if (GETPOST('type') == Facture::TYPE_CREDIT_NOTE){
            $parentInvociId = GETPOST('fac_avoir', 'int') ;

            $invoice = new Facture($db);
            $invoice->fetch($parentInvociId);
            ...
            if($no === count($invoice->lines)){
                $error++;
                setEventMessages($langs->trans("NoLinesToBill", $langs->transnoentitiesnoconv("CorrectInvoice")), null, 'errors');
            }
