...
if ($action == 'valid' && $user->rights->facture->creer) {
	...
	if ($error) {
        ...
        // Mahmoud Kamal Edit && $invoice->type !== Facture::TYPE_CREDIT_NOTE
        // add this line to check if it also credit note
		...
		if ($invoice->getRemainToPay() > 0) {
			$res = 1;
		} elseif($invoice->type !== Facture::TYPE_CREDIT_NOTE){
	...
	if (!$error && $res >= 0) {
        ...
        // Mahmoud Kamal Edit && $invoice->type !== Facture::TYPE_CREDIT_NOTE
        // عشان يعمل الفاتوره مدفوعه سواء فى حالة انها مرتجع
        if ($invoice->type === Facture::TYPE_CREDIT_NOTE) {
            //Mahmoud Kamal
            // هنا بيرجع القيمه بالسالب ف ضربتها فى سالب عشان يدهالى موجب واكل عليها الشرط
            $remaintopay = $remaintopay * -1;
        }
...
// Mahmoud Kamal . return invoic type
if ($action == 'invoiceType' && $user->rights->facture->creer) {
...
if ($action == 'creditnote' && $user->rights->facture->creer) {
    ...
    // هنا بضيف خطوه زياده عشان اعرف فى كميات كتاحه ولا لا
    // Mahmoud Kamal add
    $parentInvociId = $invoice->id;
    $condiction = true;
    $no =0;

    foreach ($invoice->lines as $line) {
            if($parentInvociId !== 0){
                $tempsql = 'SELECT qty  FROM '.MAIN_DB_PREFIX.'facturedet  WHERE fk_product = '.$line->fk_product.' AND fk_facture = '. $parentInvociId;
                $tempresult = $invoice->db->query($tempsql);
                $obj = $invoice->db->fetch_row($tempresult);
                $parentQty = $obj[0];
                $tempsql = 'SELECT SUM(qty) FROM '. MAIN_DB_PREFIX. 'facturedet WHERE fk_facture IN(SELECT rowid FROM '.MAIN_DB_PREFIX.'facture WHERE fk_facture_source = '.$parentInvociId.' AND rowid <>'.$line->fk_facture.' ) AND fk_product = '.$line->fk_product;
                $tempresult = $invoice->db->query($tempsql);
                $obj = $invoice->db->fetch_row($tempresult);
                $sumOfQty = $obj[0] ?? 0;
                $condiction = ($parentQty - $sumOfQty) > 0 ;
                if(!$condiction){
                    $no += 1;
                }
        }
	...
	if (!empty($conf->stock->enabled) && $conf->global->$constantforkey != "1") {
        ...
        // Mahmoud Kamal Comment this line
...
if ($action == "updateqty") {
    ...
    // هنا بضيف خطوه زياده عشان اعرف العكمية الى دخلها اكبر ولا اصغر ولا اقل
    // Mahmoud Kamal add
    $parentInvociId = $invoice->fk_facture_source ?? 0;
    $condiction = true;
    // end Mahmoud Kamal

    foreach ($invoice->lines as $line) {

		if ($line->id == $idline /*add by Mahmoud Kamal*/) {
            // Mahmoud Kamal add
            if($parentInvociId !== 0){
//              SELECT SUM(llx_facturedet.qty) FROM llx_facturedet WHERE fk_facture IN(SELECT rowid FROM `llx_facture` WHERE fk_facture_source = 182) AND fk_product = 1
                ...
                $condiction = ($parentQty - ($number + $sumOfQty) ) >= 0 ;
            }
...
print '<input type="hidden" value = "'.$invoice->type.'" id = "invoc_type2">';
	...
	if (is_array($invoice->lines) && count($invoice->lines)) {
		...
		foreach ($tmplines as $line) {
			...
			if (empty($_SESSION["basiclayout"]) || $_SESSION["basiclayout"] != 1) {
                ...
                // Mahmoud Kamal  Add datatoincordec
                $htmlforlines .= '<td id= "datatoincordec" data-qty = "' . $line->qty.'" class="right">';
